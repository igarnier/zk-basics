...
module Abelian_group
    (N : Name)
    (X : Generator)
    (G : Intf.Abelian_group with type t = X.t) =
struct
  open X
  open G
...
module Monoid (N : Name) (X : Generator) (G : Intf.Monoid with type t = X.t) =
struct
  open X
  open G
...
module Ring (N : Name) (X : Generator) (G : Intf.Ring with type t = X.t) =
struct
...
module Module
    ...
    (R : Generator)
    (X : Generator)
    (M : Intf.Module with type t = X.t and type R.t = R.t) =
struct
  open X
  ...
  let () =
    add_test (n "scalar_mul_unit") Crowbar.(map [ X.gen ] scalar_mul_unit)
end
